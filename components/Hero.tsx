    ...
    <section className="relative pt-44 pb-32 px-6 overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-20">
        <div className="flex-[1.2] text-center lg:text-start">
          ...
          <h1 className="text-4xl sm:text-6xl lg:text-8xl font-black text-slate-900 leading-[0.95] mb-8 tracking-tighter">
            {t.hero.title.split(' ')[0]} <span className="gradient-text">{t.hero.title.split(' ')[1]}</span> <br/>{t.hero.title.split(' ').slice(2).join(' ')}
          </h1>
          <p className="text-lg sm:text-xl text-slate-600 mb-12 max-w-2xl mx-auto lg:mx-0 leading-relaxed font-medium">
            {t.hero.subtitle}
          </p>
        ...
        <div className="flex-1 relative">
          <div className="relative z-10 rounded-[40px] overflow-hidden shadow-[0_32px_64px_-16px_rgba(79,70,229,0.2)] group">
            ...
            />
            <div className="absolute bottom-0 left-0 right-0 p-10 bg-gradient-to-t from-slate-900 via-slate-900/40 to-transparent text-white text-start">
              <div className="flex items-center gap-2 mb-3">
                <span className="px-3 py-1 bg-indigo-600 rounded-md text-[10px] font-black uppercase tracking-widest">{t.hero.flagship}</span>
                <span className="text-xs font-bold text-indigo-300">{t.hero.downloads}</span>
              </div>
              <h3 className="text-3xl font-black mb-2">{t.hero.appTitle}</h3>
              <p className="text-sm opacity-80 max-w-sm">{t.hero.appDesc}</p>
            </div>
          </div>
          
          {/* Floating Badge */}
          <div className={`absolute -bottom-10 ${locale === 'ar' ? '-right-10' : '-left-10'} glass-card p-6 rounded-3xl shadow-2xl hidden md:block animate-bounce-slow`}>
